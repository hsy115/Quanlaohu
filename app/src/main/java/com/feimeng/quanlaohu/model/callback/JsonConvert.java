 ...
 */
package com.feimeng.quanlaohu.model.callback;
    ...
    private T parseParameterizedType(Response response, ParameterizedType type) throws Exception {
        ...
        if (rawType != LzyResponse.class) {
            ...
            if (typeArgument == Void.class) {
                ...
                response.close();
                int status = lzyResponse.status;
                if (status == 0) {
                    MyToast.show(App.getInstance(), lzyResponse.msg);
                    //throw new IllegalStateException("错误代码：" + code + "，错误信息：" + lzyResponse.message);
                    throw new IllegalStateException("错误信息：" + lzyResponse.msg);
                }else {
                    return (T) lzyResponse;
                }
                //这里的0是以下意思
                //一般来说服务器会和客户端约定一个数表示成功，其余的表示失败，这里根据实际情况修改
//                if (code == 0 || code == 810002 || code == 1 || code == 2 || code == 3) {
//                    //noinspection unchecked
//                    return (T) lzyResponse;
//                } else if (code == 900009) {
//                    throw new IllegalStateException("sign签名为空");
//                } else if (code == 900008) {
//                    throw new IllegalStateException("固定参数timestamp为空");
//                } else if (lzyResponse.msg != null) {
